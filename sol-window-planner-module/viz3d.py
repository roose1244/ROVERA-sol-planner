...
Renders:
  - Mars terrain as a 3D surface (colour = elevation)
  - The planned route as a red line hovering just above the surface
  - Water-deposit pins as cyan diamond markers with hover tooltips
  - A start marker
...
def render_3d(elev, tclass, route, deposits, start, tau,
              save="demo/plan_3d.html",
              title="Sol-Window Planner — interactive 3D"):
    surf = go.Surface(
    ...
    zs = np.array([elev[r, c] for r, c in route.path]) + 3.0
    ...
    dep_z = np.array([elev[int(r.row), int(r.col)]
                      for _, r in deposits.iterrows()]) + 8
    ...
    fig.update_layout(
        title=dict(
            text=f"{title}<br><sup>ETA {route.eta_sols:.1f} sols · "
                 f"{route.total_m/1000:.1f} km · idled {route.idle_sols} sols"
                 f" through storms</sup>", x=0.5),
        template="plotly_dark",
        scene=dict(
            xaxis_title="col (px)", yaxis_title="row (px)",
            zaxis_title="elev (m)",
