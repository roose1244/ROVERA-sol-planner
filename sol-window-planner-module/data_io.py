...
def load_elevation(tile: str | None = None, shape=(256, 256), seed=0) -> np.ndarray:
    ...
    z = (
        40 * np.sin(3 * x) * np.cos(2 * y)
        + 12 * np.sin(9 * x + 1.3) * np.sin(7 * y)
        + 2 * rng.standard_normal(shape)
    )
    # A prominent ridge to make routing interesting (worth going around)
    z += 60 * np.exp(-((x - 0.55) ** 2) / 0.01)
